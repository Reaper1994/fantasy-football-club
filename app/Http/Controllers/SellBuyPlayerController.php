...
class SellBuyPlayerController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $sell_buy_players = SellBuyPlayer::where(['sell' => 1, 'buy' => 0])->with('player')->get();
        return view('teams.sell-buy-player',compact(['teams', 'sell_buy_players']));
    }
    ...
    public function sell(Request $request, int $id) :JsonResponse
    {
        ...
        $sell_buy_players = SellBuyPlayer::where(['player_id' => $id, 'sell' =>  $request->input('sell'), 'buy' => 0])->first();
        ...
        try {
            ...
            if ($sell_buy_players) {
                ...
                return response()->json(['message' => 'Player sell price updated to ' . $request->input('amount') . ' successfully']);
            } else {
                // Create a new record
    ...
    public function buy(Request $request): JsonResponse
    {
        ...
        $sell_buy_players = SellBuyPlayer::where(['player_id' => $request->player_id, 'sell' => 1, 'buy' => 0])->first();
        ...
        try {
            ...
            if ($sell_buy_players) {
                ...
                if ($sell_buy_players->amount < $team->balance ) {
                    ...
                    return response()->json(['message' => 'Player ' . $player->name . ' acquired successfully']);
                } else {
                    return response()->json(['message' => 'Sorry selected team does not have sufficient balance to acquire this player!']);
                }           
